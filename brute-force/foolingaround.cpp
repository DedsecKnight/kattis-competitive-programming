...
TAGS: troll, don't waste your time on this problem, brute-force
EXPLANATION:
- Approach: 
 + Write a brute force Sieve that will generate all possible number of stones that can be taken in 1 turn
 + By simulating all possible N, it is possible to see that there are less than 400 possible N where Bob will win
 => Generate all possible win value for Bob in a brute-force program, and copy-paste it to your solution file
 + Finally, for each test case, check if the given value is winnable for Bob
END ANNOTATION
*/
#include <bits/stdc++.h>
using namespace std;
