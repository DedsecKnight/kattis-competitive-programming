...
void prayGod() {
    ...
    cin >> grid;
    vec<vec<int>> same(n, vec<int>(m, 1));

    ll ret = 0;

    for (int i = 1; i < n; i++) {
        for (int j = 0; j < m; j++) {
            if (grid[i][j] == grid[i - 1][j]) same[i][j] = same[i - 1][j] + 1;
        }
    }

    auto process = [&](int i, int l, int r) -> void {
        int sz = r - l + 1;
        vec<int> left(sz, l - 1);
        vec<int> right(sz, r + 1);
        vec<int> st;
        for (int j = l; j <= r; j++) {
            while (!st.empty() && same[i][st.back()] >= same[i][j]) {
                st.pop_back();
            }
            if (!st.empty()) {
                left[j - l] = st.back();
        ...
        for (int j = l; j <= r; j++) {
            ret += (right[j - l] - j) * 1ll * (j - left[j - l]) * same[i][j];
        }
    };
