...
vec<vec<int>> rev_graph(MAXN);
vector<bool> visited(MAXN, false);

void dfs(int curr) {
    visited[curr] = true;
    for (int neighbor : rev_graph[curr]) {
...
void prayGod() {
    for (int i = 2; i < MAXN; i++) {
        ...
        rev_graph[nxt].push_back(i);
    }
    dfs(1);
    vec<int> ok;
    for (int i = 1; i < MAXN; i++) {
        if (visited[i]) ok.push_back(i);
    }
    ...
    while (t--) {
        int a, b;
        cin >> a >> b;
        int l = lower_bound(ok.begin(), ok.end(), a) - ok.begin();
        int r = upper_bound(ok.begin(), ok.end(), b) - ok.begin() - 1;
        cout << max(r - l + 1, 0) << '\n';
    }
}
