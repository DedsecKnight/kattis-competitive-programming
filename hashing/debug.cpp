...
constexpr int inv2mod = 500000004;
...
void prayGod() {
    ...
    for (int w = max_sz; w > 1 && ret == -1; w--) {
        vec<vec<ll>> hash(n), rev_hash(n);
        for (int i = 0; i < n; i++) {
            ll curr_hash = 0;
            for (int j = w - 1; j >= 0; j--) {
                curr_hash = (curr_hash * 2 % mod + (grid[i][j] - '0')) % mod;
            }
            hash[i].push_back(curr_hash);
            for (int j = w; j < m; j++) {
                curr_hash = (curr_hash - (grid[i][j - w] - '0') + mod) % mod;
                curr_hash = (curr_hash * inv2mod % mod + (grid[i][j] - '0') * p2[w - 1] % mod) % mod;
                hash[i].push_back(curr_hash);
            }
            ...
            rev_hash[i].push_back(curr_hash);
            for (int j = m - w - 1; j >= 0; j--) {
                curr_hash = (curr_hash - (grid[i][j + w] - '0') + mod) % mod;
                curr_hash = (curr_hash * inv2mod % mod + (grid[i][j] - '0') * p2[w - 1] % mod) % mod;
                rev_hash[i].push_back(curr_hash);
            }
            reverse(rev_hash[i].begin(), rev_hash[i].end());
        }
        ...
        for (int j = 0; j <= m - w; j++) {
            ...
            for (int i = 0, l = 0, r = -1; i < n; i++) {
                ...
                int L = i - p[i], R = i + p[i] - (!(w % 2));
                if (w % 2 == 0 || hash[L][j] == rev_hash[R][j]) {
                    while (L >= 1 && R + 1 < n && hash[L - 1][j] == rev_hash[R + 1][j]) {
                        p[i]++, L--, R++;
                    }
                    if (R > r) l = L, r = R;
                    if (2 * p[i] + (w % 2) >= w) {
