...
void prayGod() {
    while (true) {
        ...
        cin >> n;
        if (!n) break;
        vector<vector<P>> polygon(2, vector<P>(n));
        cin >> polygon;

        for (int i = 1; i <= n; i++) {
            for (int j = 0; j < 2; j++) {
                polygon[j][i % n] = polygon[j][i % n] - polygon[j][0];
            }
        }

        auto check_rotation = [&](int i, int j) -> bool {
            auto doti = polygon[0][i].dot(polygon[1][i]);
            doti *= doti;

            auto dotj = polygon[0][j].dot(polygon[1][j]);
            dotj *= dotj;

            auto prod_disti = polygon[0][i].dist2() * polygon[1][i].dist2();
            auto prod_distj = polygon[0][j].dist2() * polygon[1][j].dist2();

            return doti * prod_distj == dotj * prod_disti;
        };

        auto check_scale = [&](int i, int j) -> bool {
            return polygon[0][i].dist2() * polygon[1][j].dist2() == polygon[0][j].dist2() * polygon[1][i].dist2();
        };
        ...
        for (int i = 0; i < n; i++) {
            for (int j = i + 1; j < n; j++) {
                similar = similar && check_rotation(i, j) && check_scale(i, j);
            }
        }

        cout << (similar ? "similar" : "dissimilar") << '\n';
    }
}
