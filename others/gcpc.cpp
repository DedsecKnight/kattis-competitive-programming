...
struct Team {
    int idx, solved, penalty;
    Team() { solved = 0, penalty = 0; };
...
void prayGod() {
    int n, m; cin >> n >> m;
    vector<Team> teams(n);
    for (int i = 0; i<n; i++) teams[i].idx = i;
    auto comp = [&](int a, int b) {
        return teams[a] < teams[b];
    };
    set<int, decltype(comp)> better(comp);
    for (int i = 0; i<m; i++) {
        int t,p; cin >> t >> p;
        t--;
        better.erase(t);
        teams[t] += p;
        if (t && teams[0] < teams[t]) better.insert(t);
        while (!t && !better.empty() && teams[*better.begin()] <= teams[t]) better.erase(better.begin());
        cout << len(better) + 1 << '\n';
    }
}
