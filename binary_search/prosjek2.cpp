...
void prayGod() {
    ...
    vec<double> pref(a.begin(), a.end());
    partial_sum(pref.begin(), pref.end(), pref.begin());

    double low = 1.0, high = *max_element(a.begin(), a.end()), best = 0.0;
    for (int d = 0; d < 30; d++) {
        double mid = low + (high - low) / 2;
        double min_thus_far = 1e9 + 10;
        bool found = false;
        for (int i = 0; i < n; i++) {
            double curr = pref[i] - mid * i;
            if (min_thus_far != 1e9 + 10 && min_thus_far <= curr - mid) {
                found = true;
                break;
            }
            if (i - k + 1 >= 0) {
                min_thus_far = min(min_thus_far, pref[i - k + 1] - mid * (i - k + 2));
            }
        }
        if (found) {
            best = mid;
