...
void prayGod() {
    ...
    auto find_forest_diameter = [&](int start) {
        dist[start] = 0;
        deque<int> q{start};
        ...
        set<int, decltype(comp)> seen(comp);
        seen.insert(start);
    ...
    for (int i = 0; i < n; i++) {
        if (!visited[i]) {
            pq.push(find_forest_diameter(i));
        }
    }
    ...
    while (len(pq) > 1) {
        ...
        pq.push(max(((first + 1) >> 1) + ((second + 1) >> 1) + 1, first));
    }
