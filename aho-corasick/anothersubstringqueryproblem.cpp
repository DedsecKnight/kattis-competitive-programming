...
struct AhoCorasick {
    TrieNode *root;
    vector<int> word_size, idx_list;
    vector<vector<int>> pos;

    static const int SMALLEST_ASCII = 'a';
    ...
    void populate_dictionary(string s) {
        ...
        if (ptr->end_idx == -1) {
            ptr->end_idx = len(word_size) - 1;
            idx_list.push_back(len(word_size) - 1);
        } else
            idx_list.push_back(ptr->end_idx);
        pos.push_back({});
    }
    ...
    TrieNode *get_exit_link(TrieNode *ptr) {
        ...
        if (ptr->end_idx != UNDEFINED) return ptr->exit_link = ptr;
        TrieNode *curr_ptr = ptr;
        while (true) {
            curr_ptr = curr_ptr->link;
            if (curr_ptr->exit_link != nullptr) return ptr->exit_link = curr_ptr->exit_link;
            if (curr_ptr == root || curr_ptr->end_idx != UNDEFINED) return ptr->exit_link = curr_ptr;
        }
    }

    void search_pattern(string s, vector<pair<string, int>> &queries) {
        int s_size = len(s);
        TrieNode *ptr = root;
        ...
        for (int i = 0; i < s_size; i++) {
            ...
            while (true) {
                ...
                pos[curr_ptr->end_idx].push_back(i - word_size[curr_ptr->end_idx] + 2);
                curr_ptr = curr_ptr->link;
            }
        }
        ...
        for (int i = 0; i < len(queries); i++) {
            int idx = idx_list[i];
            if (len(pos[idx]) < queries[i].second)
                cout << -1 << '\n';
            else
                cout << pos[idx][queries[i].second - 1] << '\n';
        }
    }
};
...
void prayGod() {
    ...
    for (int i = 0; i < n; i++) readPair(queries[i]);

    AhoCorasick aho;
    for (const pair<string, int> &q : queries) {
        aho.populate_dictionary(q.first);
    }

    aho.build_link();
    aho.search_pattern(s, queries);
}
