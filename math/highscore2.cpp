...
void H() {
    int t; cin >> t;
    while (t--) {
        ll d; vector<ll> scores(3);
        read(scores, 3); cin >> d;

        sort(scores.begin(), scores.end());
        
        ll ret = 7 * scores[0];
        for (int i = 0; i<2; i++) ret += scores[i] * scores[i];
        ret += (scores[2] + d) * (scores[2] + d);

        for (int i = 0; i<100 && d > 0; i++) {
            sort(scores.begin(), scores.end());
            scores[0]++;
            d--;
            sort(scores.begin(), scores.end());
            ll new_score = 7 * scores[0];
            for (int j = 0; j<2; j++) new_score += scores[j] * scores[j];
            new_score += (scores[2] + d) * (scores[2] + d);
            ret = max(ret, new_score);
        }
