...
public class BucketBrigade {
    ...
    public void run() throws IOException {
        while (hasNext()) {
            int n = nextInt();
            double v = nextDouble(), x = nextDouble(), f = nextDouble(), t = nextDouble();

            double requiredTime = Math.max(1.0, Math.ceil(v / x));
            x = (double) v / requiredTime;

            double ret = (x / f) * requiredTime + n * (t + x / f) + (t * 2 + x / f) * (requiredTime - 1);
            out.println(ret);
