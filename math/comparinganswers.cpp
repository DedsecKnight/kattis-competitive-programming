...
struct Matrix {
    typedef Matrix M;
    vector<vector<T>> d{};
    Matrix(const vector<vector<T>> &m_) : d{move(m_)} {}
    M operator*(const M &m) const {
        M a;
        rep(i, 0, len(d)) rep(j, 0, len(d))
            rep(k, 0, len(d)) a.d[i][j] += d[i][k] * m.d[k][j];
        return a;
    }
    vector<T> operator*(const vector<T> &vec) const {
        vector<T> ret(len(d));
        rep(i, 0, len(d)) rep(j, 0, len(d)) ret[i] += d[i][j] * vec[j];
        return ret;
    }
    M operator^(ll p) const {
        assert(p >= 0);
        M a, b(*this);
        rep(i, 0, len(d)) a.d[i][i] = 1;
        while (p) {
            if (p & 1) a = a * b;
            b = b * b;
            p >>= 1;
...
struct Freivalds {
    int n_iters_;
    Freivalds(int n_iters) : n_iters_{n_iters} {}
    bool run(const Matrix<int> &A, const Matrix<int> &B, const Matrix<int> &C) {
        for (int _ = 0; _ < n_iters_; _++) {
            vector<int> Bx(len(B.d)), Cx(len(C.d));
            for (int i = 0; i < len(Bx); i++) {
                Bx[i] = rand() % 2;
            }
            for (int i = 0; i < len(Cx); i++) {
                Cx[i] = Bx[i];
            }
...
void prayGod() {
    ...
    while (cin >> n) {
        ...
        cin >> a >> c;
        Matrix<int> A{a}, C{c};
        printVerdict(Freivalds(10).run(A, A, C));
    }
}
