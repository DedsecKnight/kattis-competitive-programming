...
void prayGod() {
    ...
    vec<number> basis(160);
    vec<number> a_vec(m);
    vec<int> ret(m, -1);

    auto hex_to_bin = [&](const string &hex) -> number {
        number ans;
        int id = 159;
        for (char c : hex) {
    ...
    auto representable = [&](number v, bool add_to_basis = false) -> bool {
        for (int d = 0; d < 160; d++) {
            if (v.test(d) == 0) continue;
            if (basis[d].none()) {
                if (add_to_basis) basis[d] = v;
                return false;
            }
            v ^= basis[d];
        }
    ...
    auto is_zero_vector = [&](const number &v) -> bool {
        return v.none();
    };
    ...
    for (int i = 0; i < m; i++) {
        a_vec[i] = hex_to_bin(a[i]);
    }

    for (int j = 0; j < n; j++) {
        auto bin_vec = hex_to_bin(b[j]);

        if (representable(bin_vec, true) || is_zero_vector(bin_vec)) {
            zero_possible = true;
        }

        for (int i = 0; i < m; i++) {
            bool ok = is_zero_vector(a_vec[i]) ? zero_possible : representable(a_vec[i]);
            if (ret[i] == -1 && ok) {
                ret[i] = j + 1;
            }
        }
