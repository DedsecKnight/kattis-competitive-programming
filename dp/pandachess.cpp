...
void prayGod() {
    ...
    int sz = 0;
    umap<string, int> mp_idx;

    auto find_or_add = [&](const string &s) {
        if (present(mp_idx, s)) return mp_idx[s];
        graph.emplace_back();
    ...
    for (int i = 0; i < m; i++) {
        string u, v;
        cin >> u >> v;
        auto uid = find_or_add(u), vid = find_or_add(v);
        if (!present(seen, make_pair(uid, vid))) {
            graph[uid].push_back(vid);
            indegree[vid]++;
            seen.emplace(uid, vid);
        }
    }
    ...
    vector<int> actual_rank(n);

    for (int i = 0; i < n; i++) {
        actual_rank[top_order[i]] = i;
    }
    ...
    for (int i = 0; i < n; i++) {
        ...
        cin >> s;
        if (present(mp_idx, s)) {
            curr_rank.push_back(actual_rank[mp_idx[s]]);
        } else {
            curr_rank.push_back(-1);
        }
    }

    vector<int> aux(n + 1, iinf);
    aux[0] = -iinf;
