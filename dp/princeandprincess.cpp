...
void prayGod() {
    ...
    for (int _ = 1; _<=t; _++) {
        ...
        read(a, p); readIdx(b, q);
        sort(b.begin(), b.end());
        
        vector<int> map_idx(p, -1);
        for (int i = 0; i<p; i++) {
            int low = 0, high = q-1;
            while (low <= high) {
                int mid = low + (high - low)/2;
                if (b[mid].first == a[i]) {
                    map_idx[i] = b[mid].second;
                    break;
                }
                else if (b[mid].first > a[i]) high = mid-1;
                else low = mid+1;
            }
        }
        ...
        for (int i =0; i<p; i++) {
            ...
            lis.insert(map_idx[i]);
            auto it = lis.upper_bound(map_idx[i]);
